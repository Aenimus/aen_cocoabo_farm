...
item_group libram_worth() {
	...
	foreach ig_lib in worth
		print(worth[ig_lib].source.to_string() + ": " + ig_lib);
	float best = 0;
	float current;
	item_group chosen;
	foreach ig_lib in worth {
...
string libram_today(boolean assess) {
	if (assess) {
		...
		set_property("_aen_libramToday", libram);
	}
	return get_property("_aen_libramToday");
}
...
boolean libram_rare_run() {
	...
	while (true) {
		...
		while (summons + 1 > ig_lib.get_summons()) {
			...
			print("before summons: " + summons + " after summons: " + ig_lib.get_summons(), "purple");
		}
