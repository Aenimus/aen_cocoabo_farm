...
import "aen_cocoabo_farm\scripts\aen_shortcuts.ash";
import "aen_cocoabo_farm\scripts\aen_item_groups.ash";
...
buffer funksling(item [int] funks, item it) {
	...
	if (!$skill[Ambidextrous Funkslinging].have()) {
		b = throw_item(funks[0]);
		clear(funks);
	} else if  (count(funks) == 2) {
...
void set_temp_properties(string [string] prop_values) {
	foreach prop, value in prop_values {
...
item convert_item(string str) {
	return str.to_item();
}

skill convert_skill(string str) {
	return str.to_skill();
}

familiar convert_fam(string str) {
	return str.to_familiar();
}

boolean convert_have(string type, string str) {
	if (type == "item") return convert_item(str).have();
	else if (type == "skill") return convert_skill(str).have();
	else if (type == "familiar") return convert_fam(str).have();
	return false;
}

boolean convert_fetch(string type, string str) {
	if (type == "item") return convert_item(str).fetch();
	return false;
}

boolean convert_use(string type, string str) {
	if (type == "item") return convert_item(str).use();
	else if (type == "skill") return convert_skill(str).use();
	else if (type == "familiar") return convert_fam(str).use();
	return false;
}

int convert_amount(string type, string str) {
	if (type == "item") return convert_item(str).item_amount();
	return -1;
}

void inebriety_check() {
...
void cocoabo_farm_set(string bool) {
	print("Checkpoint reached: _aen_cocoabo_farm is " + bool + ".", "blue");
...
void cocoabo_setup_set(string bool) {
	print("Checkpoint reached: _aen_cocoabo_setup is " + bool + ".", "blue");
	set_property("_aen_cocoabo_setup", bool);
}

boolean cocoabo_free_runs() {
	return get_property("_aen_free_runs").to_boolean();
}

void cocoabo_free_runs_set(string bool) {
	print("Checkpoint reached: _aen_free_runs is " + bool + ".", "blue");
	set_property("_aen_free_runs", bool);
}

boolean cocoabo_bander_runs() {
	return get_property("_aen_bander_runs").to_boolean();
}

void cocoabo_bander_runs_set(string bool) {
	print("Checkpoint reached: _aen_bander_runs is " + bool + ".", "blue");
	set_property("_aen_bander_runs", bool);
}
...
void set_choices() {
	...
	int [int] set_choice;
	set_choice[71] = 3; // A Journey to the Center of Your Mind; 3 is Great Trip
	...
	set_choice[1059] = 1; // Helping Make Ends Meat; unlocks skeleton store
...
string camera_monster_string() {
	string check = get_property("aen_camera_monster");
	if (check == "") abort("Read and run aen_inital.ash first.");
	return check;
}

monster camera_monster() {
	string check = get_property("aen_camera_monster");
	if (check == "") abort("Read and run aen_inital.ash first.");
